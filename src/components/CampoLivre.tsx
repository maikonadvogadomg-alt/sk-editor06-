...
export default function CampoLivre({ onBack }: CampoLivreProps) {
  ...
  const sendMessage = useCallback(async (textOverride?: string) => {
    ...
    try {
      ...
      // Com chave → via proxy do servidor (auto-detecta provedor, sem CORS)
      const resp = await fetch("/api/ai/forward", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ apiKey: cleanKey, model: apiModel || undefined, messages: fullHistory, maxTokens: 16384 }),
        signal: controller.signal,
      });
