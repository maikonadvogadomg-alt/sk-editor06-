...
type SKTheme = "default" | "netflix" | "vercel";
...
function App() {
  ...
  const [theme, setTheme] = useState<SKTheme>(() => (localStorage.getItem("sk-theme") as SKTheme) || "default");
  ...
  const themeClass = theme === "netflix" ? "sk-theme-netflix"
                   : theme === "vercel"  ? "sk-theme-vercel"
                   : "";

  const THEMES: { id: SKTheme; label: string; icon: string }[] = [
  ...
  const toggleBtn = (
    <div style={{ position: "fixed", top: 8, right: 8, zIndex: 99999, display: "flex", gap: 4, alignItems: "center" }}>
      {/* Seletor de tema */}
      <div style={{ display: "flex", background: "rgba(0,0,0,0.6)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 20, overflow: "hidden" }}>
        {THEMES.map(t => (
          <button
            key={t.id}
            ...
            style={{
              ...
              background: theme === t.id ? "rgba(255,255,255,0.15)" : "transparent",
              color: theme === t.id ? "#fff" : "rgba(255,255,255,0.4)",
      ...
      <button
        ...
        style={{
          ...
          boxShadow: "0 1px 6px rgba(0,0,0,0.25)", opacity: 0.85,
        }}
  ...
  if (showCampoLivre) {
    return <div className={themeClass} style={filterStyle}>{toggleBtn}<CampoLivre onBack={() => setShowCampoLivre(false)} /></div>;
  }

  if (showAssistenteJur) {
    return <div className={themeClass} style={filterStyle}>{toggleBtn}<AssistenteJuridico onBack={() => setShowAssistenteJur(false)} /></div>;
  }

  if (!currentProject) {
    return (
      <div className={themeClass} style={filterStyle}>
        {toggleBtn}
